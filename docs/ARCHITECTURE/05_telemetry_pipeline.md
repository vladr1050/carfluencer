...
**Map buckets** are **0.001°** cells with **separate** `w_moving` and `w_stopped` counts (shared SQL via `DeviceLocationHeatmapBuckets`). Intensity per layer uses **γ** (`TelemetryHeatmapConfig`) and **normalization** `max` \| `p95` \| `p99` (`HeatmapIntensityNormalizer`): counts above the percentile cap map to peak intensity so a single hotspot does not flatten mid-density cells. Admin Filament heatmap and the advertiser SPA use **sequential** palettes (Viridis-style moving, orange sequential stopped); **both** = two heat layers, not a blended gradient. **γ** is set in **Telematics → Heatmap → Heatmap display** (key `telemetry_heatmap_intensity_gamma`); env fallback `TELEMETRY_HEATMAP_INTENSITY_GAMMA` (default **1.55**).
