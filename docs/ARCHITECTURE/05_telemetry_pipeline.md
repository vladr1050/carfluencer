...
When `TELEMETRY_HEATMAP_DRIVER=database` (default in `config/telemetry.php`), `GET /api/advertiser/heatmap` returns **`map`** (rollup or legacy buckets; viewport + zoom + `mode`), **`debug`** (caps, gamma, viewport sample counts), and **`summary_metrics`** (period KPIs from `daily_impressions` + `stop_sessions`, with optional GPS gap-fill when daily rows exist). **Summary KPIs do not use bbox/zoom or map mode**; without daily aggregates the default response is `data_source: insufficient_aggregates` (optional raw KPI fallback: `TELEMETRY_HEATMAP_ADVERTISER_RAW_SUMMARY_FALLBACK`). See **`docs/ARCHITECTURE/06_heatmap_rollup.md`** (Advertiser API section).
