...
<head>
    ...
    <style>
        ...
        .map-img { width: 100%; max-height: 420px; object-fit: contain; border: 1px solid #ddd; }
...
<section>
    ...
    @if(!empty($drivingImageBase64))
        <img class="map-img" src="data:image/png;base64,{{ $drivingImageBase64 }}" alt="Driving heatmap">
    @else
        <p>No driving map generated.</p>
    @endif
...
<section>
    ...
    <div class="meta">{{ $dateFrom }} — {{ $dateTo }} · {{ $vehicleCount }} vehicles</div>
    @if(!empty($parkingImageBase64))
        <img class="map-img" src="data:image/png;base64,{{ $parkingImageBase64 }}" alt="Parking heatmap">
    @else
        <p>No parking map generated.</p>
    @endif
